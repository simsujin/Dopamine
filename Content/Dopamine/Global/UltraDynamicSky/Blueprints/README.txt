...
-----
      ...
      Adding UDS to a Level: https://www.youtube.com/watch?v=Bkt3mXAjzTE
...
-----
      ...
      2. Simplified Color - Determines colors in simpler ways, by sampling an atmospheric scattering LUT volume texture for the sun and moon, along with adjustable color values. Cheaper on the material level than the sky atmosphere, and more directly customizable. The color values used by Simplified Color can be changed from the Simplified Color category. 
...
-----
      ...
      Feature Level is a setting in Basic Controls on UDS. This should be set to match the targeted rendering feature level for your project. For example, if you're building for the mobile renderer, you should be sure to change Feature Level over to Mobile. 
 
      Feature Level will be used by the system to correct for limitations of the selected level. For example, if mobile is selected, incompatible Sky Modes like Volumetric Clouds and Volumetric Aurora will be replaced with compatible modes. The modes used as replacements are exposed to the user in the Mobile category. 
...
-----
      ...
      - In the Second Cloud Layer dropdown, Two Layers can be enabled to add a second layer of clouds above the first, offset in a handful of ways using the Layer 2 settings. Note that a second layer will increase the cost of cloud rendering very significantly. 
...
-----
      ...
      1. Capture Based - The sky light will generate a cubemap by capturing the sky. By default, this method uses the real time capture option, which UDS exposes to a setting in the Sky Light category. 
...
-----
      ...
      By default, UDS applies auto exposure settings. Auto exposure is what allows the brightness of the image to adapt to the light level of the scene automatically. Some features in Unreal Engine occasionally refer to auto exposure as "eye adaptation". 
 
      These auto exposure settings can be adjusted, or disabled, from the Exposure category. 
 
      When Apply Exposure Settings is enabled, UDS will control these specific settings for exposure: 
 
      - Exposure Compensation Curve, which pushes image brightness higher or lower based on the general level of light in the image. 
      ...
      - MinEV100 and MaxEV100, which control the range of brightness that auto exposure will try to adapt to. This is controlled on UDS with the Exposure Brightness Range setting. 
...
-----
      ...
      Once you've built the level's lighting, you will still be able to make some changes to the sky settings, but note that any settings which alter the lights or move them around will break the lighting build and you'll have to build again. Also note that if your sun light is set to be static/stationary, Time of Day will not be able to change at runtime. 
...
-----
      ...
      Change the Time of Day property in Basic Controls to adjust the time of day the system will start with. You can also type a string like "3PM" or "15:45" into the Time of Day String variable to set the time that way. 
      ...
      ** 
      ...
      Controlling Time of Day: https://www.youtube.com/watch?v=YxtmFYq-ifA
 
      ** 
 
      Time of Day Control Functions 
      ...
      - The function "Set Time of Day With String" will set the time using strings formatted in a few common ways (12:34 PM, 18:30, 5AM, etc. See the function description for all options.) 
...
-----
      ...
      Again, it's important to note all of these values will be ignored if you're using the Simulation instead. See the section in these docs on the Simulation features for more information on how to control that. 
...
-----
      ...
      Manually Positioning the Sun and Moon: https://www.youtube.com/watch?v=akpugwKfyw4
 
      The variables controlled by the widgets are vectors called Sun Target and Moon Target. They can be found in the advanced dropdown of the Sun and Moon categories. 
 
-----
SUN APPEARANCE
-----
 
      The Sun category is where you'll find all the settings specifically for adjusting the appearance of the sun in the sky. 
 
      The sun is rendered as a simple radial gradient, so it has settings for scale, softness, color, and intensity. 
 
      Sun Scale will also be used to derive the correct light source angle of the sun directional light, so the softness of the shadows and size of specular highlights will be correct for the sun's size in the sky. You can scale it relative to that calculated value with Sun Source Angle Scale. 
...
-----
      ...
      The Moon category is where you'll find all the settings specifically for the appearance and phase of the moon. 
...
-----
      ...
      Volumetric Fog is a rendering feature in Unreal Engine which can render more advanced fog nearby the camera, which receives and scatters light from local light sources and can vary its color and extinction using volumetric materials or particles placed in the level. 
      ...
      Setting Up Volumetric Fog: https://www.youtube.com/watch?v=uRxEJsCjYbA
 
      ** 
 
      Global Volumetric Material 
 
      Also in the Volumetric Fog category is the option to render the Global Volumetric Material, which if enabled will allow some simple global controls of extinction and color. The global volumetric material has the following features, each of which can be enabled/adjusted from here in the Volumetric Fog category: 
      ...
      - Specific fog extinction and albedo can be applied beneath the water level. That's the water level controlled from the Water Level category on UDS. 
 
-----
DUST
-----
 
      Ultra Dynamic Sky has a Dust category, which can be used to imitate the air being full of dust/smoke/other particulates. 
...
-----
      ...
      If Color Mode is set to Sky Atmosphere, then the main controls of sky color will be found in the Sky Atmosphere category. Here are some of the key settings: 
...
-----
      ...
      The simulated location is determined using the Latitude and Longitude values. Also make sure to set the Time Zone to the correct UTC offset for the location, so that Time of Day will be interpreted correctly as local to this location. You can also select a city preset, which will apply latitude, longitude, time zone, and daylight savings settings in one step. 
 
      The year, month and day control the date of the simulation. At runtime, the date will increment forward every night at midnight, like you would expect. 
...
-----
      ...
      ** 
      ...
      Animating the Sky with Sequencer: https://www.youtube.com/watch?v=yxxdpjooPVI
      ...
      ** 
      ...
      Any variables you expose manually like that will probably not update with the keyframe at runtime, but you could try calling the relevant Static Properties function every frame of the animation, and that should work to force values to update. 
...
-----
      ...
      If you turn on the Path Tracer in a scene with Ultra Dynamic Sky, you'll face some rendering limitations of the path tracer. The path tracer uses the sky light cube map as a replacement for native sky rendering. 
...
-----
LOOPING THE VOLUMETRIC CLOUD MOVEMENT
-----
 
      If you need to make the volumetric clouds seamlessly loop, for rendering a looping movie for example, this is possible within some constraints. 
      ...
      In the Cloud Movement category: 
 
      - Set Cloud Speed to 0. You'll be using the Cloud Phase variable to control time within the loop. 
 
      - Make sure Cloud Direction is set to be along a single axis (0, 90, 180, or 270) 
 
      - Set Formation Change Speed to 1. (Alternatively you could use 0.5, but the loop would need to be twice as long.) 
 
      In the Volumetric Clouds category: 
      ...
      - Set 3D Noise Vertical Movement to 0. 
 
      With this configuration, you can keyframe the Cloud Phase variable from 0-100 to create an animation of cloud movement that loops seamlessly. (If you used Formation Change Speed of 0.5, the loop would be from 0-200 instead.) 
...
-----
      ...
      Using the Light Day/Night Toggle Component: https://www.youtube.com/watch?v=RrA4Ngk6zyQ
...
-----
      ...
      UDS has event dispatchers which are called at particular times, which your actors can use to perform actions at Sunset, Sunrise, Midnight, Hourly, Every Minute, or a Custom Time. 
      ...
      Note the setting "Time Event Dispatcher Check Period" in the Animate Time of Day category. This sets how often these time events can potentially be called. By default events are checked for every second whenever Time of Day is actively changing. 
 
      Using UDS Event Dispatchers: https://www.youtube.com/watch?v=u2TzMaZvLh4
 
      ** 
 
      Sunset and Sunrise 
 
      By default, the Sunset and Sunrise events will fire at the minute the sun crosses the horizon. But you can offset that time to suit your needs using the "Sunrise Event Offset" and "Sunset Event Offset" settings in the Animate Time of Day category. 
      ...
      ** 
      ...
      The Custom Time dispatcher won't be called at all by default. In the Animate Time of Day category, there is an empty float array called Custom Time Dispatchers. The dispatcher will be called for each value you add to this array, when Time of Day crosses that point at runtime. 
...
-----
      ...
      The Stars category has settings for adjusting the appearance and textures used for the stars in the sky material. 
...
-----
      ...
      UDS has two ways of rendering aurora effects in the sky. Both of these effects share most of the settings in the Aurora category, for colors and movement. 
      ...
      ** 
      ...
      Volumetric Aurora can be selected as a sky mode in Basic Controls. This effect is rendered using a volumetric cloud component, so it's a full 3D effect which the camera can go into/above. 
...
-----
      ...
      Using the Space Layer: https://www.youtube.com/watch?v=33W9NLothDE
      ...
      The space layer draws these additional planets into the sky material using the dbuffer. So Dbuffer Decals should be enabled in your project (or Mobile DBuffer Decals for the mobile renderer), and need to be supported by the platform, for these planets/moons to be rendered. 
...
-----
      ...
      It may be useful to save a full configuration of all the settings on UDS (or UDW) for reusing in other levels/projects, or applying at runtime with game events. You can do this using the Configuration Manager. Run the Configuration Manager from the Ultra Dynamic Sky dropdown on the level editor toolbar, or by running the editor utility directly, found in the Blueprints folder. 
 
      Managing Sky and Weather Configurations: https://www.youtube.com/watch?v=8HnVmPvs_N4
      ...
      The version number in the corner of the configuration thumbnails is the version of UDS that configuration was saved with. You can continue to use configurations saved with previous versions, but understand of course they won't contain data for variables/features added in later versions. So it may be in your interest to resave them so they contain complete data, depending on how you use them. 
 
      ** 
 
      Applying a Configuration at Runtime 
 
      To apply a configuration to UDS or UDW at runtime, call the function "Apply Sky Configuration" on UDS, or "Apply Weather Configuration" on UDW. If you need to apply both sky and weather configs at once, I recommend applying the weather config first. 
      ...
      ** 
      ...
      At runtime the override is applied on startup. Once the game is running, you can still work with the UDS/UDW variables as normal. 
 
      ** 
 
      Pin a Configuration Button to the Editor Layout 
 
      If you have several configurations you need to swap between while working on a level, there is an editor utility to help with this. The Configuration Buttons utility can be found in the Ultra Dynamic Sky dropdown on the level editor toolbar. 
...
-----
      ...
      UDS has a framework for applying modifiers to the sky, which can affect a selection of float and color properties on UDS, as well as post process settings. These modifiers can be applied or removed at runtime, faded in and out, and layered on top of each other. 
 
      Using Sky Modifiers: https://www.youtube.com/watch?v=9rhrlXNFtTc
      ...
      ** 
      ...
      - Add Modifier - This function will add a modifier if it is not already present, and fade it in over the supplied fade in time, in seconds. 
 
      - Remove Modifier - Similarly, this function will remove an existing applied modifier, and fade it out over the supplied fade out time. 
 
      - Set Modifier State - This function will set the alpha of a modifier in a way that it will not change on its own. For example, you could use this function to manually make a modifier be controlled by an arbitrarily changing value, like the player's position in the level. Note, the modifier does not need to already be applied to call this function. If it's not, this function will add it automatically. 
 
      - Clear Modifiers - Instantly clears out all applied modifiers at once. 
 
      - Set Unmodified Property Values - The blueprint, when it starts up, saves all of its unmodified properties, for the modifiers to interpolate from/to as they are added and removed. If you need to, you can call this function again at runtime to resave the current applied properties to be the new unmodified state. 
 
      ** 
 
      Time of Day Specific Modifiers 
 
      If you want a modifier to apply on its own within a specific Time of Day range, there is a feature for that as well. In the Sky Modifiers category, find the map called Time of Day Specific Modifiers. 
...
-----
      ...
      UDS has a category called Interior Adjustments, which if enabled, you can use to adjust a few things like fog and light, based on how occluded the camera is by surrounding scene collision. 
...
-----
      ...
      Player Occlusion is the name of a system on UDS which can actively monitor how occluded from the sky/weather the player camera/pawn is within interiors. In other words, how "inside" or "outside" the player is in relation to the scene collision around them. 
 
      The resulting occlusion values are utilized by features on the sky and weather: 
 
      - Interior Adjustments on UDS, which can adjust some sky properties based on occlusion. 
 
      - Sound Occlusion on UDW, which can quiet/muffle weather sounds when the player is occluded. 
 
      - Temperature calculation on UDW can optionally apply an interior temperature when occluded. 
 
      The Player Occlusion category on UDS contains the settings which define how occlusion values are determined. How much time each occlusion update takes, how many collision traces are used, what collision channel is traced against, etc. 
...
-----
      ...
      There are tools included for manually painting cloud coverage into the volumetric cloud layer. This painted coverage level is applied on top of the global Cloud Coverage value set on UDS (or by the weather state if UDW is present). 
      ...
      Using the Volumetric Cloud Painter: https://www.youtube.com/watch?v=ZakOT4YEcWE
      ...
      ** 
      ...
      Changing Between Multiple Cloud Paint Containers 
 
      If you have multiple cloud paint containers in your level, and you want to change which one is active, you should call the Activate and Deactivate functions on the actor. You can call this at runtime, or from the details panel in the editor. 
 
-----
WATER LEVEL
-----
 
      The Water Level category on UDS has settings to define the height of water surfaces in your level. This water level can be used by the following features across UDS and UDW: 
 
      - A caustic refraction effect, applied to the sun and moon directional light. Enable this using "Render Water Caustics" on UDS, and configure it with the options in the Water Level category. 
 
      - Volumetric fog extinction and albedo values for water. This is part of the Global Volumetric Material, configured in the Volumetric Fog category on UDS. 
 
      - In the Water Level category on UDW, you can enable "Use UDS Water Level" which will apply the water level to weather systems like weather particles and sound occlusion. 
 
      The Global Water Level variable defines a universal minimum water level, like for an ocean. This should be set to the Z height of the ocean water surface in your level. 
 
      ** 
 
      Water Body Classes 
 
      If you have a variety of water levels throughout your map, like for oceans, lakes and rivers, this is what the Water Body Classes array is for. If you add an actor class to this array, any actors of that class found in the level will be used to locally affect water level. This is done by rendering their static mesh components to a render target. If using the Water plugin, you shouldn't need to configure Water Body Classes, as it has the Water plugin's water body classes for Lake, Ocean and River selected by default. 
 
      If you need to update the water level target when water bodies are loaded/unloaded in a level, you can call the function Update Water Level Target on UDS to force an update. 
...
-----
      ...
      - Checkboxes to mask the component if raining/snowing or not. This is dependent on the presence of UDW. If UDW isn't being used, these will have no effect. 
...
-----
      ...
      Using the Onscreen Controls Widget: https://www.youtube.com/watch?v=qlwQbAMuC7Q
...
-----
      ...
      Note this focuses on the cost of UDS' features, not general engine/renderer features which aren't specific to UDS. General UE resources on optimizing performance of engine features (dynamic shadows, global illumination, etc) will apply the same when using UDS. 
 
      Sky Mode 
 
      The most GPU performance relevant setting on the whole system is certainly Sky Mode. The most expensive option here is of course Volumetric Clouds. 2D Dynamic Clouds is significantly cheaper to render, and Static Clouds is somewhat cheaper still. No Clouds obviously has the most minimal cost (outside of the Space mode). 
      ...
      Volumetric Cloud Rendering Mode - Has an effect on the cost of the cloud rendering. If looking to minimize the performance cost of the clouds on the GPU, make sure to use either of the modes which favor performance. 
      ...
      Note that enabling volumetric fog has a very significant rendering cost. In addition, UDS' own option to enable its global volumetric material will increase that cost somewhat. 
...
-----
      ...
      You can check if there are updates available for UDS by going to the update history document, linked on the home page of this documentation utility. 
      ...
      How you update will depend on if UDS is still in its default location (Content/UltraDynamicSky). 
 
      ** 
 
      If UDS is in its default location: 
 
      With UDS where the launcher expects it to be, you can do the update in one simple step. 
      ...
      If the launcher has any problem with adding or doesn't download the latest version, try selecting Remove Local Content on UDS and trying again. This will usually fix launcher issues like that. 
 
      If you have moved UDS to a different location in your project: 
 
      With UDS not in its default location, the process is more involved. You will have to perform the same move in another project and then copy it over manually, as the editor will have to adjust all the asset references within the folder to match where it is in the target project. 
      ...
      Open the target project and the update should be applied cleanly. 
 
      ** 
 
      After Installing an Update 
 
      With an update to UDS successfully installed, I would recommend reading the full change logs for any updates you got. You can find these in the Update History document linked on the home page of this utility. The change log will be the place where you'll find any info about work you may need to do in your project to support the new version. 
 
-----
MODIFYING UDS BLUEPRINTS
-----
 
      You may have a need to alter some blueprint logic within a UDS class or add your own. If this is needed, I recommend doing your modifications to a child of the class in question. That way your work is not lost when you need to update UDS. 
...
-----
      ...
      To set up weather, first make sure you have an Ultra Dynamic Sky actor in your level. The weather system will not work without one. If you haven't used Ultra Dynamic Sky yet, you should get started with the Sky section of the documentation instead. 
      ...
      Adding Ultra Dynamic Weather to a Level: https://www.youtube.com/watch?v=IhfBdzQ55Yw
...
-----
      ...
      The Weather Settings Presets are the preconfigured weather states like Thunderstorm, Partly Cloudy, and Blizzard. They're used for the Weather variable in Basic Controls, as well as by the Change Weather function, Random Weather Variation, Weather Override Volumes, and Radial Storms.
...
-----
      ...
      Using the Change Weather Function: https://www.youtube.com/watch?v=EHQExxUHEcE
 
      When Change Weather is called, it will immediately start transitioning to the new weather preset over the transition length. This new global weather state will be applied indefinitely, until another state change is called for. 
...
-----
      ...
      Setting Up Random Weather Variation: https://www.youtube.com/watch?v=jPSUK5SHk_U
...
-----
      ...
      Season is a float from 0 to 4, representing the progression from each season to the next, starting with Spring. The whole values generally represent the middle of the season (0 = Mid Spring, 1 = Mid Summer, etc) as this is where that season's values (for random weather or temperature) will be applied purely. Values in between represent the transition from one season to another. So for example, 0.5 would be half Spring and half Summer. 
...
-----
      ...
      Using the Manual Weather State: https://www.youtube.com/watch?v=AglCC4Gq_Og
...
-----
      ...
      Using Weather Override Volumes: https://www.youtube.com/watch?v=3PtE8Cc7urs
      ...
      The location used to query the WOV for its affect on weather is determined by the Control Point Location Source, in Basic Controls on UDW. 
...
-----
      ...
      Using Radial Storms: https://www.youtube.com/watch?v=3CHbEALEVx0
      ...
      ** 
      ...
      Radial Storm Spawning 
 
      On UDW, in the Radial Storm Spawning category, you'll find settings to enable periodic randomized spawning of Radial Storm actors during play. UDW will spawn radial storms one at a time, placing them and moving them so they will pass over the player's location. The weather preset the storms use can be determined by the current season, similar to Random Weather Variation. 
 
      ** 
 
      The Radial Storm actor can also be spawned in manually or placed in the level directly. You don't need to rely on UDW's spawning logic if you don't want to. The functions on the Radial Storm actor for controlling it at runtime include: 
 
      - Fade In Storm - Starts the storm fading, from 0 alpha up to 1, over the fade duration. You can call this after spawning a Radial Storm at runtime. 
 
      - Fade Out Storm - Similar to the Fade In function, but fades the storm out over time, and destroys the actor at the end of the fade. 
 
      - Move Storm Over Time - Call this function to have the storm move across the level, to a target position, over a duration. Optionally, this function can also fade out and destroy the actor at the end of the movement path. 
...
-----
      ...
      The Rain Particles category on UDW is where you can disable or adjust the effect of falling rain drops, controlled by the Rain value in the current weather state.
...
-----
      ...
      Configuring Weather Particle Collision: https://www.youtube.com/watch?v=fPtvq9D_vu4
      ...
      ** 
      ...
      If you have a moving space you need to block particles where the latency of the particle collision is a problem, for example an interior of a vehicle driven by the player, there is a component you can use in that case. The Weather Particles Kill Sphere can be added to an actor to kill any weather particles inside the sphere with zero latency. Note that this is separate from collision, and is applied by the niagara system uniquely as part of its update process. For this reason, there can never be more than one kill sphere component active at a time. If you need to use them in multiple pla
ces, you will need to manage activating and deactivating them to ensure this. 
...
-----
      ...
      ** 
      ...
      Manually Spawn a Lightning Flash 
      ...
      ** 
      ...
      Making Random Lightning Flashes Strike Specific Actors 
 
       There is some simple support for making the randomly spawned lightning flashes strike actors in the level. 
...
-----
      ...
      ** 
      ...
      Adding Surface Weather Effects to a Material: https://www.youtube.com/watch?v=vShN5jBt4dk
      ...
      ** 
      ...
      Using the Material Snow/Dust Reorient Component: https://www.youtube.com/watch?v=b9nYa_lOvTU
...
-----
      ...
      ** 
      ...
      In your material, search the material palette for Weather and select Dynamic Landscape Weather Effects V3. 
      ...
      Plug static bool nodes into the Apply Snow/Dust and Apply Wetness/Puddles inputs, to enable the effects you need for your project. For example, if you only need puddles, but never snow/dust, you could plug True into Apply Wetness/Puddles and False into Apply Snow/Dust, to reduce the complexity of the material. 
 
      Setting Up Dynamic Landscape Weather Effects: https://www.youtube.com/watch?v=csT7Jlz4ccU
      ...
      ** 
      ...
      If you do have existing Displacement for the underlying surface, note the input "Displacement Input is World Scale". This determines how the displacement input is interpreted. By default it is true, which means the displacement input is a world scale value, scaled and offset by the function to maintain that world scale. If you already have your displacement working the way you want, you can plug False into "Displacement Input is World Scale" to keep the function from modifying it. 
      ...
      ** 
      ...
      If you have other landscape layers for things like mud or sand which you want to also apply the snow/dust trail effect to, there is support for this. 
 
      First, in the material, give the DLWE function an input for Custom Trail Surface Mask. This should be a 0-1 mask for where to apply the effect in addition to the snow/dust coverage. So it would usually come from landscape layer samples. 
 
      In the DLWE Interaction Settings asset, add the physical material for the landscape layers affected to "Custom Trail Surface Physical Materials". This will configure the interaction component to draw the trails against that phys mat, even if no snow/dust coverage is present. 
 
      The depth of the custom surface will range from 0 to the Snow Depth, scaled by the Custom Trail Surface Mask. 
 
      Also note, the depth of the custom trails will apply most convincingly using nanite tessellation. This is because the trails will deform the actual mesh. With the non-nanite depth effect, the parallax occlusion cannot distort the landscape's underlying textures, since they are mapped before reaching the DLWE function. 
...
-----
      ...
      ** 
      ...
      This function outputs a 0-1 value for each season. As the season value changes on UDW, each of these four values will peak at 1. The four values will always total 1. There is also a function, Season Color Blend, which will handle blending 4 color inputs using the current season. 
...
-----
      ...
      Using Weather Mask Actors and Components: https://www.youtube.com/watch?v=XmxdzRjBL10
 
      Weather Mask Brush 
 
       The Weather Mask Brush will draw a simple shape or a painted texture, selectable using the variable "Brush". The brush actor can also be scaled non-uniformly and rotated on the Z axis. Because the system works by drawing these shapes into a render target, the shapes are always projected straight up and down on the Z axis. 
      ...
      When finished, make sure to click Save, for the painted mask to be saved to a texture asset. This asset will be put in a folder called UDS_LevelData, in the folder where the level is saved. 
...
-----
      ...
      If you need to adjust which sounds are turned on at runtime, I'd recommend leaving the sound effects enabled, and setting their exposed Volume variables at runtime. To toggle all the sound effects at runtime, you can call "Set Enable Weather Sound Effects". 
...
-----
      ...
      You can also place the UDS Occlusion Volume actor, found in Blueprints/Occlusion, to force specific spaces to be fully occluded and always attenuate weather sounds. Note the option in Player Occlusion for "Occlusion Sampling Mode" which affects if these volumes will be sampled. 
...
-----
      ...
      In addition to the weather sounds, UDW has a system for playing environment sounds. This would be for general ambient audio in the outdoor environment like birds, insects, city sounds, etc. 
      ...
      An environment sound asset can be selected in the Sound Effects category on UDW. There is an example included for forest ambience, but the system is built for you to author new ones with your own sounds. 
 
      ** 
 
      Making an Environment Sound Asset 
 
       Environment Sounds exist as data assets, of the type UDS_Environment_Sound. You can make a new one by making a new data asset from the content browser and selecting that class. 
 
      The data asset mainly serves to point to the metasound source asset which is what will be played for this environment sound. It also has settings to scale the volume, and parameter overrides for the metasound source. 
 
      ** 
 
      Making a Metasound Source Asset for Environment Sound 
 
       The metasound source asset you make for environment sound needs to be authored in some specific ways. 
      ...
      The sound should also be configured to output to the audio bus used for weather sound. In Source settings, make sure Enable Bus Sends is true, and Enable Base Submix is disabled. Then add a Post-Effect Bus Send which outputs to the audio bus "UDS_Weather_AudioBus". 
 
      ** 
 
      Controlling Your Metasound Source with Time/Weather 
      ...
      I'd recommend looking at included Forest Example metasound source to see how this is intended to be set up. 
 
      ** 
 
      Controlling Environment Sounds at Runtime 
 
      Selecting an Environment Sound on UDW will set what sound is active on begin play. If you need to change the environment sound to another at runtime, you can by calling the function Change Environment Sound on UDW. 
...
-----
      ...
      To sample the current weather state values from your own blueprint classes, there are functions you can use. 
...
-----
      ...
      UDW has event dispatchers which your classes can bind to trigger events based on changes to the weather state.
...
-----
      ...
      By default, wind intensity will have a variation applied to it, to simulate gusts of wind. This variation comes from a stream of perlin noise, which pushes wind intensity higher and lower. 
...
-----
      ...
      A Wind Directional Source is an engine component which can affect a few things, like chaos cloth and speedtree assets. UDW has an option, enabled by default, to make and control a Wind Directional Source component with the current wind speed and direction. 
...
-----
      ...
      If you want the player camera to shake when exposed to maximum wind speeds, UDW has a feature for that. You can find the toggle for it in the Wind Camera Shake category. 
 
      You can adjust the scale of the shake applied, as well as the range of Wind Intensity from the weather state which will trigger the shake. 
...
-----
      ...
      Using the Puddle Fluid Volume: https://www.youtube.com/watch?v=nyrvyv1UzSs
...
-----
APPLYING WATER LEVEL TO WEATHER
-----
 
      If you need weather features to be affected by water bodies in your level, these is an option for this, called Use UDS Water Level, in the Water Level category on UDW. The actual water level which will be used is configured from UDS, from its own Water Level category. See the documentation entry in the sky section about that. 
 
      Here is all the water level can affect on UDW: 
...
-----
      ...
      - The sun itself needs to be low enough for the rainbow to be visible above the horizon, or the camera has to be high enough to see the rainbow beneath them. 
...
-----
      ...
      There is a widget blueprint included which you can use to display the current weather on UDW. It's called UDW_Current_Weather_Display, and it can be found in the widget designer library under Ultra Dynamic Sky Widgets. 
...
-----
      ...
      Adding Dripping Particles to Meshes: https://www.youtube.com/watch?v=OaVh1-ECQ9M
...
-----
      ...
      Rain Drip Spline is an actor which you can place into your level to have a line or curve spawn rain droplets which fall based on the amount of rain in UDW's weather state. This can be used to make something like the edge of a building roof appear to drip water during rain. You can also additionally enable icicle meshes hanging from the spline for cold weather. 
...
-----
      ...
      If you want to create icicles hanging from the edges of meshes in your level in cold weather, see the entry on the Rain Drip Spline actor, which has an optional feature for rendering icicle meshes along its spline. 
 
-----
WEATHER OCCLUSION VOLUME
-----
 
      The Weather Occlusion Volume is a volume texture which contains a low resolution cache of the area around the camera position, and how occluded each point is from the weather/wind. It does this by sampling the global distance field, so it requires distance fields to be enabled and supported. 
...
-----
      ...
      Animating Weather with Sequencer: https://www.youtube.com/watch?v=3YG23-9jhr4
      ...
      ** 
      ...
      To spawn a lightning flash from a sequence, add UDW to the sequence and create an Event>Trigger track on UDW. Add a keyframe to the track, and right click it to see its properties. For its Endpoint, search for Flash Lightning under Quick Bind. With that function bound, you can configure the inputs for the lightning flash (it's custom location, seed, etc) from the keyframe's properties. Make sure to also check Call in Editor in the properties if you want the lightning flash to be previewed in the sequence editor. 
...
-----
      ...
      Startup 
       ...
       The function Startup Sky is the one which contains all of UDS' startup logic, shared when run from Construction Script in the editor, and on begin play when starting the level. It's also called to restart the sky at runtime, like when applying a configuration or changing sky mode during play. 
 
      The Startup Sky function handles all of the logic which only needs setting up once at startup, and it does the first run of the Update functions. 
      ...
      General Organization 
       ...
       For finding functions relevant to any of the specific features on UDS, take note first that the functions are categorized generally by feature. For example, the function which applies static properties for the lens flare is called "Static Properties - Lens Flare" and is in the Lens Flare category. 
...
-----
      ...
      UDW has a function similar to Startup Sky on UDS. On UDW it's called Start Weather System. 
 
      This handles creating the necessary objects, like weather settings objects, and starting systems like random weather variation. It also does the first run of the update functions. 
      ...
      Monitor Local Weather Changes 
       ...
       This function runs after the local weather state is generated, looking for any values which have changed past a threshold. Any that have changed get a set of relevant functions bound to a dispatcher called in Update Active Variables, to apply changes to anything which responds to that state value. 
...
-----
      ...
      The functions which can be bound do things like applying the weather preset, handling transitions if active, applying the random weather state, and incrementing the global material state sim. 
 
      Local Weather State 
 
      The construction of the local weather state is handled in much the same way. The binding function is called as needed to bind functions to the Construct Local Weather dispatcher, which is called from Update Current Global and Local Weather State. 
 
      The Local Weather bindings handle things like applying weather override volumes, radial storms, and changes above the cloud layer. 
 
      In the Update Active Variables function, almost all of the properties set there will be sampling the values on the Local Weather State object, since that represents the weather state at the player's location. 
...
-----
      ...
      Weather Particles 
       ...
       The Rain, Dust, Snow, and Wind Debris system assets can be found in the Particles folder. 
      ...
      The rest of the emitters sample the arrays randomly to get a valid particle path, to control their location, velocity, and lifetime. 
...
-----
      ...
      - If you see this problem on specific platforms, it may be because they don't support real time sky light capture. I'd recommend changing Sky Light Mode to Cubemap with Dynamic Color Tinting for a good dynamic option that works on any platform. 
...
-----
      ...
      Some properties, if you just set them directly at runtime, will have no effect on what they control. In most cases this would be because they are what the system calls static properties. They are applied when the system starts up, but it doesn't waste resources applying them actively since they don't typically need to change based on time or weather. 
...
-----
      ...
      If using some types of water or ocean shaders, you might notice in cloudy weather that the sun or moon directional lights appear to affect the material despite being blocked by clouds. This would be because the single layer water shading model tests the intensity and color of the active directional light without factoring in light functions. 
...
-----
VOLUMETRIC CLOUDS HAVE HARD MESH INTERSECTIONS
-----
 
      If using the default settings, you might notice volumetric clouds intersections with meshes look hard and low quality. 
      ...
      If you need your clouds to render well over the landscape/objects in your scene, the fix is simple. In the Volumetric Clouds category on UDS, just change the Volumetric Cloud Rendering Mode to one of the settings which prioritize Mesh Intersections. 
 
-----
TRANSLUCENT MATERIALS AREN'T BLOCKED BY CLOUDS
-----
 
      You may notice translucent materials rendering in front of volumetric clouds. This is because translucent materials require additional configuration to be blocked by volumetric clouds. 
...
-----
      ...
      If you see a hard line where fog/atmosphere suddenly starts on scenery at a particular distance from the camera, the problem is probably the position of the sky atmosphere relative to the scene. 
...
-----
      ...
      If you have volumetric fog enabled, you might notice pixelation or sample quality artifacts detracting from quality. 
...
-----
      ...
      - Make sure your landscape has World Static simple collision which responds to queries and blocks the Visibility channel.  If you're not using a landscape actor, but other kinds of meshes, note the extra steps for that involving physical materials, in the DLWE documentation entry. 
...
-----
      ...
      If you're having trouble with weather  particles not being blocked by objects that they seemingly should, here are a few things to check, based on what you have Particle Collision Mode set to: 
...
-----
      ...
      By default, the sound effects on UDW use a simple system of sound occlusion to dim and muffle weather sounds when inside interiors. That's toggled from the Sound Occlusion category. 
